/* AgentsWorkspace — master-detail shell for /agents/[id] and /agents/select: left agent list +
   (Config editor | "select an agent" prompt). Tab state lives in ?tab=. Ported from screen_agents.jsx. */
"use client";
...
export function AgentsWorkspace({ id }: { id?: string }) {
  ...
  const setTab = (t: string) => {
    ...
    sp.set("tab", t);
    if (id) router.replace(`/agents/${id}?${sp.toString()}`);
  };
          ...
          <div style={{ flex: 1, overflow: "auto", padding: "0 12px 12px" }}>
            {(agents ?? []).map((a) => (
              <AgentCard
                ...
                onDeleted={() => {
                  if (a.id === id) router.push(AGENTS_SELECT_ROUTE);
                }}
              />
            ))}
          </div>
